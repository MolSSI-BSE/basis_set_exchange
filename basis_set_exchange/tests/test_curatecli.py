...
def _test_curatecli_cmd(cmd):
    ...
    # Python to run
    cmd = (
        "{} {} ".format(sys.executable, os.path.join(cli_dir, "bsecurate_cli.py")) + cmd
    )
    cmd = cmd.split(" ")
...
test_files1 = [
    "6-31G.0.table.json",
    "ahlrichs/def2-ECP.1.element.json",
    "dunning/cc-pV5+dZ-add.1.json",
]
...
def test_curatecli_compare_1():
    output = _test_curatecli_cmd(
        "compare-basis-sets 6-31g 6-31g --version1 0 --version2 0"
    )
    assert b"No difference found" in output

    output = _test_curatecli_cmd(
        "compare-basis-sets 6-31g 6-31g --version1 0 --version2 0 --uncontract-general"
    )
    assert b"No difference found" in output

    output = _test_curatecli_cmd(
        "compare-basis-sets 6-31g 6-31g --version1 0 --version2 1"
    )
...
def test_curatecli_compare_files(filename1, filename2, expected):
    ...
    output = _test_curatecli_cmd(
        "compare-basis-files {} {} --uncontract-general".format(file1, file2)
    )
    if expected:
        assert b"No difference found" in output
    else:
    ...
    output = _test_curatecli_cmd(
        "compare-basis-files {} {} --uncontract-general".format(file2, file1)
    )
    if expected:
        assert b"No difference found" in output
    else:
