...
_reader_map = {
    "turbomole": {"display": "Turbomole", "extension": ".tm", "reader": read_turbomole},
    "gaussian94": {"display": "Gaussian94", "extension": ".gbs", "reader": read_g94},
    "nwchem": {"display": "NWChem", "extension": ".nw", "reader": read_nwchem},
    "dalton": {"display": "Dalton", "extension": ".mol", "reader": read_dalton},
    "molcas": {"display": "MolCAS", "extension": ".molcas", "reader": read_molcas},
    "cfour": {"display": "CFOUR", "extension": ".c4bas", "reader": read_genbas},
    "genbas": {"display": "Genbas", "extension": ".genbas", "reader": read_genbas},
    "gbasis": {"display": "GBasis", "extension": ".gbasis", "reader": read_gbasis},
}
...
def read_formatted_basis_file(
    file_path, basis_fmt=None, encoding="utf-8-sig", validate=False, as_component=False
):
    ...
    if basis_fmt is None:
        for k, v in _reader_map.items():
            ...
            raise RuntimeError(
                "Unable to determine basis set format of '{}'".format(file_path)
            )
    else:
