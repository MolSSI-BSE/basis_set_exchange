...
def _parse_electron_lines(basis_lines, bs_data):
    ...
    shell_blocks = helpers.partition_lines(
        basis_lines[1:], lambda x: x[0].isalpha(), min_size=2
    )

    for sh_lines in shell_blocks:
        element_sym, shell_am = helpers.parse_line_regex(
            am_line_re, sh_lines[0], "Element sym, shell am"
        )
        ...
        element_data = helpers.create_element_data(
            bs_data, element_Z, "electron_shells", key_exist_ok=True
        )
...
def _parse_ecp_lines(basis_lines, bs_data):
    ...
    for pot_lines in ecp_blocks:
        ...
        if len(pot_lines) == 1:
            ...
            if not nelec_re.match(pot_lines[0]):
                raise RuntimeError(
                    "Unknown block with single line. Perhaps it's an empty block? Line: "
                    + pot_lines[0]
                )

            element_sym, n_elec = helpers.parse_line_regex(
                nelec_re, pot_lines[0].lower(), "ECP: Element sym, nelec"
            )

            element_Z = lut.element_Z_from_sym(element_sym, as_str=True)
            element_data = helpers.create_element_data(
                bs_data, element_Z, "ecp_electrons", create=int
            )
            element_data["ecp_electrons"] = n_elec
        else:
            element_sym, pot_am = helpers.parse_line_regex(
                am_line_re, pot_lines[0], "ECP: Element sym, pot AM"
            )

            element_Z = lut.element_Z_from_sym(element_sym, as_str=True)
            element_data = helpers.create_element_data(
                bs_data, element_Z, "ecp_potentials", key_exist_ok=True
            )
    ...
    for el, v in bs_data.items():
        if "ecp_potentials" in v and "ecp_electrons" not in v:
            raise RuntimeError(
                "Number of ECP electrons not specified for element {}".format(el)
            )
