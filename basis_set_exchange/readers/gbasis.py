...
def read_gbasis(basis_lines):
    ...
    element_sections = helpers.partition_lines(
        basis_lines, element_entry_re.match, min_size=4
    )
    ...
    for element_lines in element_sections:
        ...
        element_sym, basis_name, _ = helpers.parse_line_regex(
            element_entry_re, element_lines[0], "Element entry: sym:basis:pattern"
        )
        ...
        shell_blocks = helpers.partition_lines(
            element_lines[2:], lambda x: x[0].isalpha()
        )

        # We know how many blocks there should be
        if (max_am + 1) != len(shell_blocks):
            raise RuntimeError(
                "Different number of blocks for element {}. Expected {}, found {}".format(
        ...
        for shell_lines in shell_blocks:
            shell_am, nprim, ngen = helpers.parse_line_regex(
                shell_info_re, shell_lines[0], "Shell: AM, nprim, ngen"
            )
            ...
            if shell_am in found_am:
                raise RuntimeError(
                    "Duplicate AM for element {}: AM {} already found".format(
                        element_sym, shell_am[0]
                    )
                )
