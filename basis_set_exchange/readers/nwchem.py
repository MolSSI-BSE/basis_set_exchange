    ...
    for pot_lines in ecp_blocks:
        ...
        if len(pot_lines) == 1:
            ...
            if not nelec_re.match(pot_lines[0]):
                raise RuntimeError("Unknown block with single line. Perhaps it's an empty block? Line: " + pot_lines[0])
...
def read_nwchem(basis_lines):
    ...
    basis_sections = helpers.partition_lines(
        basis_lines, lambda x: x.lower() == "end", min_blocks=1, max_blocks=2, include_match=False
    )
