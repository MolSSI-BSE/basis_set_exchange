    ...
    for n in range(nsh):
        ...
        for p in range(nprim):
            ...
            for g in range(ngen):
                ...
                if r > 0.0:
                    print(
                        "Coefficient {:3}: {:20} {:20} -> {:16.8e}".format(p, c1, c2, r)
                    )
                max_rdiff = max(max_rdiff, r)
...
def potentials_difference(p1, p2):
    ...
    for n in range(np):
        ...
        for p in range(nprim):
            ...
            if r > 0.0:
                print(
                    "   Gaussian Exponent {:3}: {:20} {:20} -> {:16.8e}".format(
                        p, e1, e2, r
                    )
                )
            max_rdiff = max(max_rdiff, r)
            ...
            if r > 0.0:
                print(
                    "          R Exponent {:3}: {:20} {:20} -> {:16.8e}".format(
                        p, e1, e2, r
                    )
                )
            max_rdiff = max(max_rdiff, r)
            ...
            for g in range(ngen):
                ...
                if r > 0.0:
                    print(
                        "         Coefficient {:3}: {:20} {:20} -> {:16.8e}".format(
                            p, c1, c2, r
                        )
                    )
                max_rdiff = max(max_rdiff, r)
...
def basis_comparison_report(bs1, bs2, uncontract_general=False):
    ...
    for k, v in bs2["elements"].items():
        ...
        if "electron_shells" in v and "electron_shells" in bs1_el:
            max_rdiff_el = max(
                max_rdiff_el,
                shells_difference(v["electron_shells"], bs1_el["electron_shells"]),
            )
        if "ecp_potentials" in v and "ecp_potentials" in bs1_el:
            ...
            if int(nel1) != int(nel2):
                print(
                    "Different number of electrons replaced by ECP ({} vs {})".format(
                        nel1, nel2
                    )
                )
                max_rdiff_ecp = float("inf")
...
def compare_basis_against_file(
    basis_name,
    src_filepath,
    file_type=None,
    version=None,
    ...
    return basis_comparison_report(
        src_data, bse_data, uncontract_general=uncontract_general
    )
