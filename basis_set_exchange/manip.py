...
def prune_shell(shell, use_copy=True):
    ...
    for ex in ex_groups:
        ...
        for g in ex_coeff:
            ...
            if len(nonzero) > 1:
                raise RuntimeError(
                    "Exponent {} is duplicated within a contraction".format(ex[0])
                )
        ...
        for am in all_am:
            ...
            for sh in el["electron_shells"]:
                ...
                if ft1 not in ft2 and ft2 not in ft1:
                    raise RuntimeError(
                        "Cannot make general contraction of different function types"
                    )
...
def remove_free_primitives(basis, use_copy=True):
    ...
    for k, el in basis["elements"].items():
        ...
        for sh in el["electron_shells"]:
            ...
            contracted_columns = [
                idx for idx, c in enumerate(coefficients) if not _is_single_column(c)
            ]
...
def optimize_general(basis, use_copy=True):
    ...
    for eldata in basis["elements"].values():
        ...
        for sh in elshells:
            ...
            single_columns = [
                idx for idx, c in enumerate(coefficients) if _is_single_column(c)
            ]
                ...
                for row_idx in range(nprim):
                    if float(col[row_idx]) != 0.0:
                        if row_idx in all_row_idx:
                            raise RuntimeError(
                                "Badly-formatted basis. Row {} makes duplicate shells".format(
                                    row_idx
                                )
                            )
...
def geometric_augmentation(basis, nadd, use_copy=True, as_component=False, steep=False):
    ...
    if nadd < 1:
        raise RuntimeError(
            "Adding {} functions makes no sense for geometric_augmentation".format(nadd)
        )
    ...
    if as_component:
        ...
        basis["elements"] = {
            k: {} for k, v in basis_copy["elements"].items() if "electron_shells" in v
        }
    elif use_copy:
        basis = copy.deepcopy(basis)
    ...
    for el_z, eldata in basis_copy["elements"].items():
        ...
        for shell in eldata["electron_shells"]:
            ...
            if ref_exp == next_exp:
                raise RuntimeError(
                    "The two outermost exponents are the same. Duplicate exponents are not a good thing here. Exponent: {}".format(
                        ref_exp
                    )
                )
...
def remove_primitive(electron_shell, idx_to_remove):
    ...
    electron_shell["coefficients"] = [
        gen
        for gen in electron_shell["coefficients"]
        if any([float(c) != 0.0 for c in gen])
    ]
...
def _element_remove_diffuse(eldata, nremove):
    ...
    for shell in shells:
        ...
        if len(shell_am) > 1:
            raise RuntimeError(
                "Cannot not remove diffuse functions from fused shell: {}".format(
                    shell_am
                )
            )
...
def truhlar_calendarize(basis, month, use_copy=True):
    ...
    if month_offset > basis_max_am:  # or month_offset >= (basis_max_am+1)
        raise RuntimeError(
            "Will not remove {} diffuse functions for basis with max am = {}".format(
