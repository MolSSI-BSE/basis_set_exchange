...
def convert_formatted_basis_str(basis_in, in_fmt, out_fmt):
    ...
    basis_dict = read_formatted_basis_str(
        basis_in, in_fmt, validate=True, as_component=False
    )
    return write_formatted_basis_str(basis_dict, out_fmt)


def convert_formatted_basis_file(
    file_path_in, file_path_out, in_fmt=None, out_fmt=None, encoding="utf-8-sig"
):
