...
def create_metadata_file(output_path, data_dir):
    ...
    for meta_file_relpath in meta_filelist:
        ...
        this_filelist = [
            x
            for x in table_filelist
            if os.path.dirname(x) == base_relpath
            and os.path.basename(x).startswith(base_filename)
        ]
        ...
        for table_file in this_filelist:
            ...
            if function_types is None:
                ...
                raise RuntimeError(
                    "Differing function types across versions for " + base_filename
                )
