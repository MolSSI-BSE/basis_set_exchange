...
def _bse_cli_list_basis_sets(args):
    ...
    metadata = api.filter_basis_sets(
        args.substr, args.family, args.role, args.elements, args.data_dir
    )

    if args.no_description:
        liststr = [x["display_name"] for x in metadata.values()]
    else:
        liststr = format_columns(
            [(v["display_name"], v["description"]) for k, v in metadata.items()]
        )
...
def _bse_cli_get_info(args):
    ...
    version_lines = format_columns(
        [
            (k, v["revdate"], compact_elements(v["elements"]), v["revdesc"])
...
def _bse_cli_convert_basis(args):
    ...
    convert.convert_formatted_basis_file(
        args.input_file, args.output_file, args.in_fmt, args.out_fmt
    )
...
def _bse_cli_create_bundle(args):
    ...
    bundle.create_bundle(
        args.bundle_file, args.fmt, args.reffmt, args.archive_type, args.data_dir
    )
