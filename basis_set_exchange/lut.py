...
def amint_to_char(am, hij=False, use_L=False):
    ...
    for a in am:
        if a < 0:
            raise IndexError(
                "Angular momentum must be a positive integer (not {})".format(a)
            )
        if a >= len(amchar_map):
            raise IndexError(
                "Angular momentum {} out of range. Must be less than {}".format(
                    a, len(amchar_map)
                )
            )
...
def electron_shells_start(nelectrons, max_am=20):
    ...
    if nelectrons in special_am:
        ...
        if nelectrons != 0:
            raise RuntimeError(
                "Electrons cover a partial shell. {} electrons left".format(nelectrons)
            )
