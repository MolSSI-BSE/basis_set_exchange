...
def _header_string(basis_dict):
    ...
    header += (
        tw.fill(
            "     Version: {}  ({})".format(
                basis_dict["version"], basis_dict["revision_description"]
            )
        )
        + "\n"
    )
        ...
        if elements:
            ...
            for el in elements:
                if el not in bs_elements:
                    ...
                    raise KeyError(
                        "Element {} (Z={}) not found in basis {} version {}".format(
            ...
            basis_dict["elements"] = {
                k: v for k, v in bs_elements.items() if k in elements
            }
...
def get_references(basis_name, elements=None, version=None, fmt=None, data_dir=None):
    ...
    basis_dict = get_basis(
        basis_name, elements=elements, version=version, data_dir=data_dir
    )
...
def filter_basis_sets(
    substr=None, family=None, role=None, elements=None, data_dir=None
):
    ...
    if elements is not None:
        ...
        for basis_name, basis_data in metadata.items():
            ...
            basis_data["versions"] = {
                k: v for k, v in ver_data.items() if elements <= set(v["elements"])
            }
    ...
    if substr:
        ...
        metadata = {
            ...
            if substr in k or substr in v["display_name"]
        }
