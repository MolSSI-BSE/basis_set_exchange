    ...
    for name, data, notes in _basis_data_iter(fmt, reffmt, data_dir):
        for ver, verdata in data.items():
            ...
            ref_filename = os.path.join(
                subdir, "{}.{}.ref{}".format(filename, ver, refext)
            )
...
_bundle_types = {
    "zip": {"display": "ZIP file", "handler": _bundle_zip, "extension": ".zip"},
    "tbz": {"display": "Tar + bzip2", "handler": _bundle_tbz, "extension": ".tar.bz2"},
}
...
def create_bundle(outfile, fmt, reffmt, archive_type=None, data_dir=None):
    ...
    if archive_type is None:
        ...
        for k, v in _bundle_types.items():
            ...
            raise RuntimeError(
                "Cannot autodetect archive type from file name: {}".format(
