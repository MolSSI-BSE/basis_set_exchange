...
def chunk_list(lst, rows, cols):
    ...
    if n_elements != rows * cols:
        raise RuntimeError(
            "Cannot partition {} elements into a {}x{} matrix".format(
...
def remove_expected_line(lines, expected="", position=0):
    ...
    if position >= 0 and len(lines) <= position:
        raise RuntimeError(
            "Not enough lines. Can't test line {} when there are {} lines".format(
                position, len(lines)
            )
        )
    if position < 0 and len(lines) < -position:
    ...
    if lines[position] != expected:
        raise RuntimeError(
            "Expected line '{}', but found '{}'".format(expected, lines[0])
        )
...
def create_element_data(
    bs_data, element_Z, key, key_exist_ok=False, element_exist_ok=True, create=list
):
    ...
    if key not in bs_data[element_Z]:
        ...
        raise RuntimeError(
            "Key {} already exists in basis data for element {}".format(key, element_Z)
        )
...
def parse_line_regex(rex, line, description=None, convert_int=True):
    ...
    if not r:
        if description:
            raise RuntimeError(
                "Regex '{}' does not match line: '{}'. Regex is '{}'".format(
                    description, line, rex.pattern
                )
            )
        else:
            raise RuntimeError(
                "Regex '{}' does not match line: '{}'".format(rex.pattern, line)
            )
...
def partition_lines(
    ...
    if before > 0:
        ...
        if len(all_blocks) <= 1:
            raise RuntimeError(
                "Cannot partition lines with before = {}: have {} blocks".format(
        ...
        if len(all_blocks[0]) != before:
            raise RuntimeError(
                "Cannot partition lines with before = {}: first block has {} lines".format(
    ...
    if min_size > 0:
        for idx, block in enumerate(all_blocks):
            if len(block) < min_size:
                raise RuntimeError(
                    "Block {} does not have minimum number of lines ({})".format(
                        idx, min_size
                    )
                )
    ...
    if min_blocks and len(all_blocks) < min_blocks:
        raise RuntimeError(
            "Found {} blocks, but need at least {}".format(len(all_blocks), min_blocks)
        )
    if max_blocks and len(all_blocks) > max_blocks:
        raise RuntimeError(
            "Found {} blocks, but need at most {}".format(len(all_blocks), max_blocks)
        )
...
def read_n_floats(lines, n_numbers, convert=False, split=r"\s+"):
    ...
    while len(found_numbers) < n_numbers:
        if not lines:
            raise RuntimeError(
                "Wanted {} numbers but ran out of lines after {}".format(
                    n_numbers, len(found_numbers)
                )
            )
        if not lines[0]:
            raise RuntimeError(
                "Wanted {} numbers but found empty line after {}".format(
                    n_numbers, len(found_numbers)
                )
            )
    ...
    if len(found_numbers) != n_numbers:
        raise RuntimeError(
            "Wanted {} numbers, but found {}".format(n_numbers, found_numbers)
        )

    # Make sure these are floating point
    if not all(is_floating(x) for x in found_numbers):
        raise RuntimeError(
            "Non-floating-point value found in numbers: " + " ".join(found_numbers)
        )
...
def read_n_integers(lines, n_ints, convert=False, split=r"\s+"):
    ...
    if len(found_numbers) != n_ints:
        raise RuntimeError(
            "Wanted {} numbers, but found {}".format(n_ints, found_numbers)
        )

    # Make sure these are integers
    if not all(is_integer(x) for x in found_numbers):
        raise RuntimeError(
            "Non-integer value found in numbers: " + " ".join(found_numbers)
        )
...
def parse_matrix(lines, rows=None, cols=None, split=r"\s+"):
    ...
    for l in lines:
        ...
        if not all(is_floating(x) for x in s):
            raise RuntimeError(
                "Non-floating-point value found in matrix: " + " ".join(s)
            )
    ...
    for x in mat:
        ...
        if len(x) != len(mat[0]):
            raise RuntimeError(
                "Inconsistent number of columns: {} vs {}".format(len(x), len(mat[0]))
            )
    ...
    if rows is not None:
        ...
        if len(mat) != rows:
            raise RuntimeError(
                "Inconsistent number of rows: {} vs {}".format(rows, len(mat))
            )
    ...
    if cols is not None and len(mat[0]) != cols:
        raise RuntimeError(
            "Inconsistent number of columns: {} vs {}".format(cols, len(mat[0]))
        )
...
def parse_primitive_matrix(lines, nprim=None, ngen=None, split=r"\s+"):
    ...
    for l in lines:
        ...
        if not all(is_floating(x) for x in c):
            raise RuntimeError(
                "Non-floating-point value found in coefficients: " + " ".join(c)
            )
    ...
    for c in coefficients:
        ...
        if len(c) != len(coefficients[0]):
            raise RuntimeError(
                "Inconsistent number of coefficients: {} vs {}".format(
    ...
    if nprim is not None:
        ...
        if len(exponents) != nprim:
            raise RuntimeError(
                "Inconsistent number of primitives in exponents: {} vs {}".format(
                    nprim, len(exponents)
                )
            )
        ...
        if len(coefficients[0]) != nprim:
            raise RuntimeError(
                "Inconsistent number of primitives in coefficients: {} vs {}".format(
    ...
    if ngen is not None:
        ...
        if len(coefficients) != ngen:
            raise RuntimeError(
                "Inconsistent number of general contractions: {} vs {}".format(
...
def remove_block(lines, start_re, end_re):
    ...
    for idx, l in enumerate(lines):
        if start.match(l):
            if start_idx != -1:
                raise RuntimeError(
                    "Multiple blocks starting with '{}' found".format(start)
                )
            start_idx = idx
            break
    else:
    ...
    if i == len(lines):
        raise RuntimeError(
            "Cannot find end of block. Looking for '{}' to close '{}'".format(
                end_re, start_re
            )
        )
