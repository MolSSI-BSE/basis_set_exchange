...
from .common_testvars import (
    bs_names_sample,
    diffuse_augmentation_test_data_dir,
    steep_augmentation_test_data_dir,
    truhlar_test_data_dir,
    rmfree_test_data_dir,
)
    ...
    for slevel, sprefix in [(0, ""), (1, "s"), (2, "d")]:
        ...
        for dlevel, dprefix in [(0, ""), (1, "d"), (2, "t")]:
            if slevel == 0 and dlevel == 0:
                continue
            ref = (
                testdir.replace("un-", "un{}-".format(sprefix)).replace("aug-", "{}aug-".format(dprefix))
                + ".nw.ref.bz2"
            )
            full_ref_path = os.path.join(full_testdir, ref)
            ref_data = readers.read_formatted_basis_file(full_ref_path, "nwchem")
