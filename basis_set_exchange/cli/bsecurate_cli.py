...
def run_bsecurate_cli():
    ...
    subparsers.add_parser(
        "update-metadata", help="Update the metadata in the repository"
    )
    ...
    ########################################
    subp = subparsers.add_parser(
        "print-component-file", help="(Pretty) print the contents of a component file"
    )
    ...
    # make-diff
    subp = subparsers.add_parser(
        "make-diff", help="Find/Store the differences between two groups of files"
    )
    ...
    # compare-basis-sets
    subp = subparsers.add_parser(
        "compare-basis-sets", help="Compare two basis sets in the data directory"
    )
    subp.add_argument(
        "basis1", help="First basis set to compare"
    ).completer = cli_bsname_completer
    subp.add_argument(
        "basis2", help="Second basis set to compare"
    ).completer = cli_bsname_completer
    subp.add_argument(
        "--version1",
        help="Version of the first basis set to compare with. Default is latest",
    )
    subp.add_argument(
        "--version2",
        help="Version of the second basis set to compare with. Default is latest",
    )
    subp.add_argument(
        "--uncontract-general",
        action="store_true",
        help="Remove general contractions before comparing",
    )

    # compare-basis-files
    subp = subparsers.add_parser(
        "compare-basis-files", help="Compare two formatted basis set files"
    )
    ...
    subp.add_argument(
        "--readfmt1", help="Override the file format of file 1"
    ).completer = cli_readerfmt_completer
    subp.add_argument(
        "--readfmt2", help="Override the file format of file 2"
    ).completer = cli_readerfmt_completer
    subp.add_argument(
        "--uncontract-general",
        action="store_true",
    ...
    subp.add_argument(
        "basis", help="Name of the basis set inspect"
    ).completer = cli_bsname_completer
    subp.add_argument(
        "--version",
        help="Which version of the basis set to inspect. Default is the latest version",
    )

    # make-graph-file
    subp = subparsers.add_parser(
        "make-graph-file",
        help="Make a dot file (and png file) for a basis set file graph",
    )
    subp.add_argument(
        "basis", help="Name of the basis set inspect"
    ).completer = cli_bsname_completer
    ...
    subp.add_argument(
        "--render",
        action="store_true",
        help="Render the DOT file into a corresponding png file",
    )
    subp.add_argument(
        "--version",
        help="Which version of the basis set to inspect. Default is the latest version",
