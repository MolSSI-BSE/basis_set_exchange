...
electron_z_max_am_re = re.compile(
    r"^(\d+|{})(?:\s+(\d+))?$".format(helpers.floating_re_str)
)
shell_nprim_ngen_re = re.compile(r"^(\d+)(?:\s+(\d+))?$")

ecp_info_re = re.compile(r"^[Pp]{2}\s*,\s*([a-zA-Z]+)\s*,\s*(\d+)\s*,\s*(\d+)\s*;$")
ecp_pot_begin_re = re.compile(
    r"^(\d+)\s*;.*$"
)  # Sometime comments are after the semicolon
...
def _parse_electron_lines(basis_lines, bs_data, element_Z):
    ...
    options_lines, basis_lines = helpers.remove_block(
        basis_lines, "Options", "EndOptions"
    )
    ...
    nuc_charge, max_am = helpers.parse_line_regex(
        electron_z_max_am_re, basis_lines[0], "Electron: Z, max_am"
    )
    ...
    if (
        "ecp_electrons" in element_data
        and element_data["ecp_electrons"] != ecp_electrons
    ):
        raise RuntimeError(
            "Element Z: {} with charge {} does not match ECP electrons {}".format(
    ...
    shell_blocks = helpers.partition_lines(
        basis_lines[1:], lambda x: x.split()[0].isdecimal()
    )
    ...
    if max_am is not None and len(shell_blocks) != (max_am + 1) * (n_option_blocks + 1):
        raise RuntimeError(
            "Found {} shell blocks. Expected {}".format(
                len(shell_blocks), (max_am + 1) * (n_option_blocks + 1)
            )
        )
    ...
    for shell_lines in shell_blocks:
        nprim, ngen = helpers.parse_line_regex(
            shell_nprim_ngen_re, shell_lines[0], "Shell nprim, ngen"
        )
        ...
        if ngen is not None and ngen <= 0:
            raise RuntimeError(
                "Cannot have {} general contractions in a shell".format(ngen)
            )
        ...
        if ngen is not None and ngen != n_coefs // nprim:
            raise RuntimeError(
                "Expected {} general contractions, but found {}".format(
                    ngen, n_coefs // nprim
                )
            )
        else:
...
def _parse_ecp_lines(basis_lines, bs_data, element_Z):
    ...
    _, basis_lines = helpers.remove_block(
        basis_lines, r"^Spectral.*", r"^End\s*Of\s*Spectral.*"
    )
    ...
    if element_Z_ecp != element_Z:
        raise RuntimeError(
            "ECP element Z={} found in block for element Z={}".format(
                element_Z, element_Z_ecp
            )
        )
    ...
    pot_blocks = helpers.partition_lines(
        basis_lines[1:], ecp_pot_begin_re.match, min_size=2
    )

    if len(pot_blocks) != max_am + 1:
        raise RuntimeError(
            "Expected {} potentials, but got {}".format(max_am + 1, len(pot_blocks))
        )
    ...
    for pot_lines in pot_blocks:
        ...
        nlines = helpers.parse_line_regex(
            ecp_pot_begin_re, pot_lines[0], "ECP Potential: # of lines"
        )
        if nlines != len(pot_lines) - 1:
            raise RuntimeError(
                "Expected {} lines in potential, but got {}".format(
                    nlines, len(pot_lines) - 1
                )
            )
...
def read_molcas(basis_lines):
    ...
    element_blocks = helpers.partition_lines(
        basis_lines, lambda x: x.startswith("/"), min_size=4
    )
    ...
    for element_lines in element_blocks:
        ...
        element_sym, basis_name = helpers.parse_line_regex(
            element_head_re, element_lines[0], "Start of element line"
        )
    ...
    if len(basis_names_found) > 1:
        raise RuntimeError(
            "Multiple basis sets found in file: " + ",".join(basis_names_found)
        )
