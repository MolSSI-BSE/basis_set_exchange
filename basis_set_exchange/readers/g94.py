...
am_line_re = re.compile(
    r"^([A-Za-z]+)\s+(\d+)((?:\s+{})+)$".format(helpers.floating_re_str)
)
...
def _parse_electron_lines(basis_lines, bs_data):
    ...
    for sh_lines in shell_blocks:
        ...
        shell_am, nprim, scaling_factors = helpers.parse_line_regex(
            am_line_re, sh_lines[0], "Shell AM, nprim, scaling"
        )
        ...
        if len(scaling_factors) == 0:
            raise RuntimeError(
                "No scaling factors given for element {}: Line: {}".format(
                    element_sym, sh_lines[0]
                )
            )
...
def _parse_ecp_lines(basis_lines, bs_data):
    ...
    max_am, ecp_electrons = helpers.parse_line_regex(
        ecp_am_nelec_re, basis_lines[1], "ECP max_am, nelec"
    )
    ...
    for pot_lines in ecp_blocks:
        ...
        if not helpers.is_integer(pot_lines[1]):
            raise RuntimeError(
                "Number of lines for potential is not an integer: " + pot_lines[1]
            )
        ...
        if len(pot_lines) != (nlines + 2):
            raise RuntimeError(
                "Number of lines is incorrect. Expected {}, got {}".format(
                    nlines, len(pot_lines) - 2
                )
            )
...
def read_g94(basis_lines):
    ...
    element_sections = helpers.partition_lines(
        basis_lines, element_re.match, min_size=3
    )
